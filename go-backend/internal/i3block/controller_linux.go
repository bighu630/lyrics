...
func (c *Controller) Start() error {
	...
	// Initial PID refresh (non-critical, will retry in monitor loop)
	if err := c.refreshPID(); err != nil {
		log.Debug().Err(err).Msg("Initial i3block PID refresh failed")
	}
...
func (c *Controller) monitorLoop() {
	for {
		...
		case <-c.ticker.C:
			if err := c.refreshPID(); err != nil {
				log.Warn().Err(err).Msg("Failed to refresh i3block PID")
			}
		case <-c.stopChan:
			return
		}
...
func (c *Controller) refreshPID() error {
	...
	if len(lines) > 0 && lines[0] != "" {
		...
		if oldPID != pid {
			log.Info().Int("old_pid", oldPID).Int("new_pid", pid).Msg("i3block PID updated")
		}
	}
	...
	for _, line := range lines {
		if strings.Contains(line, "i3blocks") && !strings.Contains(line, "grep") {
			...
			if len(fields) >= 2 {
				...
				if oldPID != pid {
					log.Info().Int("old_pid", oldPID).Int("new_pid", pid).Msg("i3block PID updated (alternative method)")
				}
				return nil
			}
		}
...
func (c *Controller) SendSignalUSR1() error {
	...
	log.Info().Int("pid", pid).Msg("Successfully sent SIGUSR1 to i3block process")
	return nil
}
...
func (c *Controller) SendSignalUSR2() error {
	...
	log.Info().Int("pid", pid).Msg("Successfully sent SIGUSR2 to i3block process")
	return nil
}
